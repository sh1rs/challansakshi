...
- **Scanner scope and limits:** the seventeen candidate checks parse authored TypeScript and the built bundles as syntax trees and reject every enumerated authority class — computed or aliased global objects, non-allowlisted Chrome APIs, eval and dynamic import in every spelling, network primitives, navigation sinks including bare and aliased location, synthetic activation, and foreign URL identities — with a 42-payload adversarial regression matrix derived from six independent review rounds. Static analysis of minified bytes cannot be complete against names computed only at runtime; those residual shapes are neutralized by the shipped extension CSP for scripts, connections, images, forms, and base URLs, and the reviewed artifact of record is the tracked authored source, which the scanner fully covers. The scanner is defense in depth over reviewed code, not a substitute for review.
