...
function Workspace({ initialCaseId, language: initialLanguage = 'en', onEndSession }: WorkspaceProps & { onEndSession: (language: Language) => void }) {
  ...
  return <PublicBetaShell language={language} setLanguage={setLanguage} service="Your mobility cases" serviceHindi="आपके मोबिलिटी केस">
    <main className={styles.main}>
      ...
      <nav className={styles.navigation} aria-label={t('Mobility tools', 'मोबिलिटी उपकरण')}><a href="/review">{t('Review a document', 'दस्तावेज़ की समीक्षा')} <ArrowRight size={14} /></a><a href="/dashboard">{t('Simple checklist', 'सरल सूची')}</a><a href="/account">{t('Account availability', 'खाते की उपलब्धता')}</a><a href="/demo">{t('Try with sample data', 'उदाहरण डेटा से आज़माएँ')}</a></nav>
