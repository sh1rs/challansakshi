...
const steps: Step[] = ['start', 'records', 'reconcile', 'packet'];
  ...
  return (
    <PublicBetaShell language={language} setLanguage={setLanguage} service="TollSakshi" serviceHindi="टोल साक्षी · by ChallanSakshi" onQuickExit={quickExit} englishOnly>
      <main className={styles.main}>
        ...
        <ol className={styles.progress} aria-label={t(language, 'Toll review progress', 'टोल समीक्षा प्रगति')}>
          {steps.map((item, index) => <li key={item} className={index < progressIndex ? styles.stepDone : index === progressIndex ? styles.stepActive : ''}><span>{index < progressIndex ? '✓' : index + 1}</span>{[t(language, 'Scope', 'दायरा'), t(language, 'Records', 'रिकॉर्ड'), t(language, 'Map', 'मिलान'), t(language, 'Packet', 'पैकेट')][index]}</li>)}
        </ol>

        {step === 'start' && <section className={styles.panel} aria-labelledby="toll-start-title">
          <div className={styles.sectionTitle}><div><p className={styles.eyebrow}>{t(language, 'Choose how to use it', 'उपयोग का तरीका चुनें')}</p><h2 id="toll-start-title">{t(language, 'Manual self-review or synthetic walkthrough', 'मैन्युअल स्वयं-समीक्षा या सिंथेटिक उदाहरण')}</h2><p>{t(language, 'Real mode is tab-memory only. Synthetic cases contain no citizen data.', 'रियल मोड केवल टैब मेमोरी में है। सिंथेटिक मामलों में नागरिक डेटा नहीं है।')}</p></div><span className={styles.stepTag}>01 / 04</span></div>
          <div className={styles.choiceGrid}>
            <button type="button" className={`${styles.choice} ${mode === 'real' ? styles.choiceActive : ''}`} onClick={selectRealMode}><strong>{t(language, 'Use my own records manually', 'अपने रिकॉर्ड मैन्युअली उपयोग करें')}</strong><small>{t(language, 'No uploads, no AI, no account, no automatic filing.', 'कोई अपलोड, AI, अकाउंट या स्वचालित फाइलिंग नहीं।')}</small></button>
            <button type="button" className={`${styles.choice} ${mode === 'synthetic' ? styles.choiceActive : ''}`} onClick={selectSyntheticMode}><strong>{t(language, 'Explore fictional examples', 'काल्पनिक उदाहरण देखें')}</strong><small>{t(language, 'Different vehicle, citizen-reported two-debit pattern, and records-aligned refusal.', 'अलग वाहन, नागरिक द्वारा दर्ज दो-डेबिट पैटर्न और मेल खाते रिकॉर्ड का इनकार।')}</small></button>
            <button type="button" className={`${styles.choice} ${device === 'private' ? styles.choiceActive : ''}`} onClick={() => setDevice('private')}><strong>{t(language, 'Private device', 'निजी डिवाइस')}</strong><small>{t(language, 'Local copy/download available at the end.', 'अंत में स्थानीय कॉपी/डाउनलोड उपलब्ध।')}</small></button>
        ...
        {step === 'records' && <section className={styles.panel} aria-labelledby="toll-records-title">
          <div className={styles.sectionTitle}><div><p className={styles.eyebrow}>{mode === 'synthetic' ? t(language, 'Fictional record set', 'काल्पनिक रिकॉर्ड सेट') : t(language, 'Minimum masked facts', 'न्यूनतम मास्क तथ्य')}</p><h2 id="toll-records-title">{t(language, 'Describe the transaction—not your account', 'लेन-देन बताएँ—अपना खाता नहीं')}</h2><p>{t(language, 'Keep reader-read, debit-post, and SMS-received times distinct.', 'रीडर-रीड, डेबिट-पोस्ट और SMS-मिलने के समय अलग रखें।')}</p></div><span className={styles.stepTag}>02 / 04</span></div>
          <div className={styles.formGrid}>
            ...
            {mode === 'real' && <div className={`${styles.field} ${styles.fieldWide}`}><label className={styles.check}><input type="checkbox" checked={tagMappingConfirmed} onChange={(event) => setTagMappingSignature(event.target.checked ? currentTagMappingSignature : '')} />{t(language, 'I found the vehicle suffix below in that official FASTag account’s tag-to-vehicle mapping—not only in my memory or RC. Editing the source, tag, or vehicle suffix requires confirmation again.', 'मैंने नीचे का वाहन प्रत्यय उस आधिकारिक FASTag खाते की टैग-से-वाहन मैपिंग में पाया—केवल अपनी याद या RC में नहीं। स्रोत, टैग या वाहन प्रत्यय बदलने पर फिर पुष्टि करनी होगी।')}</label></div>}
