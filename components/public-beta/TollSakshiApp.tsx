...
type RecordGroup = 'issue' | 'transaction' | 'passing' | 'extra' | 'confirm';
...
export default function TollSakshiApp() {
  ...
  const recordGroupRefs = useRef<Partial<Record<RecordGroup, HTMLDetailsElement | null>>>({});
  ...
  const continueRecords = () => {
    if ([refs.tagSuffix, refs.vehicleSuffix, refs.transactionSuffix].some((value) => value.length > 0 && value.length !== 4)) {
      showRecordError('transaction', t(language, 'Each identifier suffix must be exactly four characters, or blank.', 'हर पहचान प्रत्यय ठीक चार अक्षर/अंक का हो, या खाली रहे।'));
      return;
    }
    if (refs.amount && !isValidAmount(refs.amount)) {
      showRecordError('transaction', t(language, 'Enter a debit amount greater than 0 and no more than ₹1,00,000, with at most two decimal places—or leave it blank.', '0 से अधिक और ₹1,00,000 तक की डेबिट राशि, अधिकतम दो दशमलव अंकों के साथ दर्ज करें—या खाली छोड़ें।'));
      return;
    }
    if (mode === 'real' && (!sourceConfirmed || !refs.issuerLabel || refs.issuerLabel === 'Not sure')) {
      showRecordError('issue', t(language, 'First choose and confirm the official account service where you independently found the debit.', 'पहले वह आधिकारिक खाता सेवा चुनें और पुष्टि करें जहाँ आपने स्वतंत्र रूप से डेबिट पाया।'));
      return;
    }
    if (mode === 'real' && !reconciliationConfirmed) {
      showRecordError('confirm', t(language, 'Review the final same-transaction confirmation after your last edit.', 'अंतिम बदलाव के बाद उसी लेन-देन की अंतिम पुष्टि फिर जाँचें।'));
      return;
    }
    ...
    <PublicBetaShell language={language} setLanguage={setLanguage} service="TollSakshi" serviceHindi="टोल साक्षी · by ChallanSakshi" onQuickExit={quickExit} englishOnly>
      <main className={styles.main}>
        ...
        {step === 'records' && <section className={styles.panel} aria-labelledby="toll-records-title">
          ...
          <details className={`${styles.recordSection} ${styles.recordDisclosure}`} ref={(node) => { recordGroupRefs.current.transaction = node; }}>
            <summary className={styles.recordSectionHeading}><span>2</span><div><strong id="record-event-title">{t(language, 'Transaction details', 'लेन-देन विवरण')}</strong><small>{t(language, 'Use last-four identifiers and keep reader, posting, and SMS times distinct.', 'अंतिम चार पहचान उपयोग करें और रीडर, पोस्टिंग और SMS समय अलग रखें।')}</small></div></summary>
            <div className={styles.recordDisclosureBody}>
            <div className={styles.formGrid}>
          ...
          <details className={`${styles.recordSection} ${styles.recordDisclosure}`} ref={(node) => { recordGroupRefs.current.passing = node; }}>
            <summary className={styles.recordSectionHeading}><span>3</span><div><strong id="record-passing-title">{t(language, 'Passing image', 'पासिंग तस्वीर')}</strong><small>{t(language, 'Record only what a supplied image appears to show.', 'केवल दी गई तस्वीर में जो दिखे वही दर्ज करें।')}</small></div></summary>
            <div className={styles.recordDisclosureBody}>
          ...
          {showExtraCheck && <details className={`${styles.recordSection} ${styles.recordDisclosure}`} ref={(node) => { recordGroupRefs.current.extra = node; }}>
          <summary className={styles.recordSectionHeading}><span>4</span><div><strong id="record-concern-title">{t(language, 'Extra check for this issue', 'इस समस्या की अतिरिक्त जाँच')}</strong><small>{t(language, 'Only fields needed for the selected issue appear here.', 'केवल चुनी समस्या के लिए आवश्यक फ़ील्ड यहाँ दिखते हैं।')}</small></div></summary>
          <div className={styles.recordDisclosureBody}>
        ...
        {step === 'packet' && <section className={styles.panel} aria-labelledby="packet-title">
          ...
          <section className={styles.preparationStatus} aria-labelledby="preparation-note-title">
            <h3 id="preparation-note-title">{t(language, 'Preparation note', 'तैयारी नोट')}</h3>
            <p><strong>{assessment.shouldPrepareIssuerNote ? t(language, 'Available for your review.', 'आपकी समीक्षा के लिए उपलब्ध।') : t(language, 'Withheld for this result.', 'इस परिणाम के लिए रोका गया।')}</strong></p>
            {assessment.shouldPrepareIssuerNote ? <>
              <p className={styles.artifactWarning}>{device === 'shared' ? t(language, 'The note stays available on screen, but copy/download controls are disabled. The page attempts to leave after about 10 minutes of inactivity.', 'नोट स्क्रीन पर उपलब्ध रहता है, लेकिन कॉपी/डाउनलोड नियंत्रण बंद हैं। लगभग 10 मिनट निष्क्रिय रहने पर पेज बाहर निकलने का प्रयास करता है।') : t(language, 'Copy/download may leave information outside this tab. TollSakshi cannot erase those copies.', 'कॉपी/डाउनलोड से जानकारी इस टैब के बाहर रह सकती है। TollSakshi उन कॉपी को मिटा नहीं सकता।')}</p>
              <div className={styles.deadline}><strong>{t(language, 'Report an incorrect deduction promptly through the official channel for your FASTag', 'अपने FASTag के आधिकारिक रास्ते से गलत कटौती तुरंत रिपोर्ट करें')}</strong><p>{t(language, 'IHMCL’s FASTag FAQ currently says to report an incorrect deduction within 40 days of the transaction date and says the chargeback process normally takes up to 20–30 working days. Treat 40 days as source-labelled IHMCL reporting guidance, not a statutory limitation period; treat 20–30 working days as a normal processing estimate, not a refund or resolution guarantee. Contact the appropriate official channel promptly and verify its current process.', 'IHMCL की FASTag FAQ वर्तमान में लेन-देन की तारीख से 40 दिनों के भीतर गलत कटौती रिपोर्ट करने और चार्जबैक प्रक्रिया में सामान्यतः 20–30 कार्यदिवस तक लगने की बात कहती है। 40 दिन IHMCL की स्रोत-लेबल मार्गदर्शिका है, कानूनी सीमा नहीं; 20–30 कार्यदिवस सामान्य प्रक्रिया अनुमान है, रिफंड या समाधान की गारंटी नहीं। उचित आधिकारिक रास्ते से तुरंत संपर्क करें और वर्तमान प्रक्रिया जाँचें।')}</p></div>
              <div className={styles.noteActions}><button type="button" className={styles.buttonSecondary} disabled={device === 'shared'} onClick={copyWorksheet}>{t(language, 'Copy note', 'नोट कॉपी करें')}</button><button type="button" className={styles.button} disabled={device === 'shared'} onClick={downloadWorksheet}>{t(language, 'Download .txt', '.txt डाउनलोड करें')}</button></div>
            </> : <div className={styles.stopCard}><h3>{t(language, 'No issuer dispute note prepared', 'कोई जारीकर्ता विवाद नोट तैयार नहीं किया गया')}</h3><p>{t(language, 'This result does not support a transaction dispute note. Follow the route above or correct the records.', 'यह परिणाम लेन-देन विवाद नोट का समर्थन नहीं करता। ऊपर का रास्ता अपनाएँ या रिकॉर्ड सुधारें।')}</p></div>}
            {artifactStatus && <p className={styles.inlineError} role="status">{artifactStatus}</p>}
          </section>

          <section className={styles.listPanel} aria-labelledby="unresolved-evidence-title">
          ...
          <div className={styles.actions}><button type="button" className={styles.buttonSecondary} onClick={() => setStep('reconcile')}>← {t(language, 'Back to map', 'नक्शे पर वापस')}</button></div>
        </section>}
      </main>
    </PublicBetaShell>
  );
