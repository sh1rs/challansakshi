...
export function PrivacyPage() {
  ...
  return <PublicBetaShell language={language} setLanguage={setLanguage} service="ChallanSakshi" serviceHindi="गोपनीयता और डेटा नियंत्रण">
    <main className={styles.infoPage}>
      <p className={styles.eyebrow}>{t(language, 'Version 2 · reviewed 29 August 2026', 'संस्करण 2 · समीक्षा 29 अगस्त 2026')}</p>
      ...
      <InfoSection title={t(language, 'Local files and real-mode answers', 'स्थानीय फ़ाइलें और रियल-मोड उत्तर')}>
        <ul>
          <li>{t(language, 'Each selected record or supplied image stays in the current browser tab and is not sent to the ChallanSakshi server or an AI model. This release does not run OCR on it.', 'हर चुना गया रिकॉर्ड या दी गई तस्वीर मौजूदा ब्राउज़र टैब में रहती है और ChallanSakshi सर्वर या AI मॉडल को नहीं भेजी जाती। यह रिलीज़ उस पर OCR नहीं चलाती।')}</li>
          ...
          <li>{t(language, 'Files and answers are held in page memory. They are not written by the real flow to localStorage, sessionStorage, cookies, a case URL, /api/analyze, or a case database.', 'फ़ाइलें और उत्तर पेज मेमोरी में रहते हैं। रियल प्रवाह उन्हें localStorage, sessionStorage, cookies, केस URL, /api/analyze या केस डेटाबेस में नहीं लिखता।')}</li>
          <li>{t(language, 'Reloading, closing the tab, or Quick exit clears the in-app copy. Shared-device mode disables this app’s summary copy, download, and print controls and attempts to leave after inactivity; use Quick exit when finished.', 'रीलोड, टैब बंद या तुरंत बाहर निकलने पर ऐप के अंदर की कॉपी साफ़ होती है। साझा-डिवाइस मोड ऐप के सारांश कॉपी, डाउनलोड और प्रिंट नियंत्रण बंद करता है और निष्क्रियता के बाद बाहर निकलने का प्रयास करता है; काम पूरा होने पर तुरंत बाहर निकलें।')}</li>
        </ul>
      </InfoSection>
      ...
      <InfoSection title={t(language, 'Synthetic demo storage', 'सिंथेटिक डेमो संग्रह')}>
        <p>
          {t(language, 'The separate ', 'अलग ')}<code>/demo</code>{t(language, ' walkthrough stores only its bundled fictional case state and reading preferences in this browser so judges and learners can resume it. Use its Reset control to remove that synthetic demo state. Never type real information into the synthetic Resolution Desk. Its optional model-rerun route accepts only a bundled fictional fixture ID and image; live model reruns are disabled in the public production release. TollSakshi’s three fictional fixtures stay in page memory and are persistently watermarked.', ' वॉकथ्रू अपने बंडल किए गए काल्पनिक केस की स्थिति और पढ़ने की पसंद इस ब्राउज़र में रखता है ताकि जज और सीखने वाले इसे फिर शुरू कर सकें। उस सिंथेटिक डेमो स्थिति को हटाने के लिए Reset उपयोग करें। सिंथेटिक Resolution Desk में असली जानकारी कभी न लिखें। उसका वैकल्पिक मॉडल-रीरन रास्ता केवल बंडल काल्पनिक fixture ID और तस्वीर स्वीकार करता है; सार्वजनिक प्रोडक्शन रिलीज़ में लाइव मॉडल रीरन बंद हैं। TollSakshi के तीन काल्पनिक fixtures पेज मेमोरी में रहते और लगातार वॉटरमार्क होते हैं।')}
        </p>
      </InfoSection>
      ...
      <InfoSection title={t(language, 'Language status', 'भाषा स्थिति')}>
        <p>{t(language, 'The real e-Challan review and these reading pages offer English and Hindi. FASTag remains English-only until its complete decision and artifact copy finishes Hindi safety review. If translated wording appears incomplete or unclear, use the English limitation and official-route text before acting.', 'रियल ई-चालान समीक्षा और ये पठन पेज अंग्रेज़ी और हिंदी देते हैं। FASTag तब तक केवल अंग्रेज़ी है जब तक उसके पूरे निर्णय और आर्टिफैक्ट पाठ की हिंदी सुरक्षा समीक्षा पूरी नहीं होती। अनुवाद अधूरा या अस्पष्ट लगे तो कार्रवाई से पहले अंग्रेज़ी सीमा और आधिकारिक-रास्ता पाठ उपयोग करें।')}</p>
      </InfoSection>
    </main>
  </PublicBetaShell>;
}
