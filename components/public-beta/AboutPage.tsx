  ...
  return <PublicBetaShell language={language} setLanguage={setLanguage} service="ChallanSakshi" serviceHindi="ChallanSakshi के बारे में">
    <main className={styles.page}>
      ...
      <header className={styles.hero}>
        <h1>{t('What is ChallanSakshi?', 'ChallanSakshi क्या है?')}</h1>
      ...
      <section className={styles.section} aria-labelledby="about-tools">
        ...
        <p className={styles.note}>{t('Need help knowing where to start?', 'समझ नहीं आ रहा कहाँ से शुरू करें?')} <a href="/guides">{t('Read the e-Challan and FASTag guides.', 'ई-चालान और FASTag के मार्गदर्शक पढ़ें।')}</a></p>
      </section>
      ...
      <nav className={styles.related} aria-label={t('Project information', 'परियोजना की जानकारी')}><a href="/privacy">{t('Privacy & data controls', 'गोपनीयता और डेटा नियंत्रण')}</a><a href="/safety">{t('Safety guidance', 'सुरक्षा मार्गदर्शन')}</a><a href="/sources">{t('Official sources', 'आधिकारिक स्रोत')}</a><a href="https://github.com/sh1rs/challansakshi" target="_blank" rel="noopener noreferrer">{t('Project source on GitHub', 'GitHub पर परियोजना का स्रोत')}</a></nav>
    </main>
  </PublicBetaShell>;
}
