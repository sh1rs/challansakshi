...
- **Use details already supplied:** task text can offer registrations, labelled references and a stated authority for one grouped review. Home-state qualifiers, uncertain issuers and unrelated narrative stay in the note; a registration prefix never supplies jurisdiction.
- **One useful next step:** evidence, service and consistency checks run concurrently on-device, then a coach highlights one question or next action. These four roles use deterministic rules. They are not language models or official verification.
- **Reviewed continuity:** selected profile corrections can update eligible unfinished drafts together. Saved-case search, a short selective brief, encrypted manual transfer and original-file fingerprint checks support resuming and explaining a case without a backend.
- **After a reply:** the complete reviewed follow-up note can become a separate preparation case. Optional related-case details require review, and the original submitted/completed history remains intact.
...
Start with [the free account setup guide](docs/mobility-account-setup.md), or use [encrypted manual case transfer](docs/mobility-portable-case.md) with no provider setup. The [multi-agent and assistance guide](docs/mobility-ai-and-assistance.md) explains the implemented roles and optional activation. The [one-service pilot record](docs/mobility-pilot-checklist.md) lists the remaining real-world evidence needed before expanding official coverage.

The [original-idea coverage map](docs/2026-09-06-idea-coverage.md) separates the implemented local journeys, provider-dependent features, synthetic demonstrations and remaining official integrations. Use the [current release record](docs/2026-09-07-mobility-live-release.md) for deployment, test and provider-availability boundaries.
...
- Original documents, message-check text and the raw reply-review source remain in page memory. Reviewed document facts, their source fingerprints and an edited request, or the reviewed reply follow-up note, can enter a mobility case only after a separate explicit private-device save. Optional account transfers require another reviewed choice. Original files and unselected reply passages are excluded. The separate opt-in `/dashboard` saves only its limited checklist metadata. The review/manual tools' shared-device mode disables their copy/download controls, provides Quick exit & clear, and attempts to leave after about 10 minutes without pointer, keyboard, input, or touch activity; the UI does not claim a background browser timer is infallible. `/mobility` does not have that shared-device mode or inactivity exit. Its copying/downloads remain available; leave saving off on a shared device and use **Clear this session** to remove working forms and reviews. Saved records, clipboard and downloaded copies require separate cleanup.
...
- No original-document server-upload surface. The dedicated message and reply tools accept bounded local text without automatic transmission or persistence; the reviewed reply note has a separate opt-in saved-case action. `/review` runs local PDF text extraction or English/Hindi OCR and identifies the reading method; `/manual/challan` remains citizen-observed. The flagship demo remains visibly synthetic.
...
- By default, real-mode answers, selected document bytes and extracted fields exist in current page memory. Separate explicit mobility-case saving retains reviewed facts and notes. They expire 90 days after the last save; physical cleanup happens on a subsequent store read or action, not while the app is closed. Reload, close, or Quick exit clears the review-page copy; it does not delete a deliberately saved case, original PDF tab, download or account copy.
...
- The document and reply review engines keep their raw inputs ephemeral. Explicitly saved mobility cases use a separate validated local store and random case IDs in URL fragments; optional account sessions use HTTP-only cookies after Google sign-in. No case data is sent to analytics or the real-case `/api/analyze` path. The document reader accepts bounded field-specific corrections; the manual compatible handoff retains its single controlled description textarea, revision-bound and safety-checked. Neither is submitted to an official service by ChallanSakshi. Automated privacy tests cover the default local reading path and the separate consented persistence boundaries.
...
- The review/manual tools' copy/download/formatted-print controls are disabled in their shared-device mode, which attempts to leave the review after about 10 minutes of inactivity. Their browser-native print is reduced to a non-sensitive warning, but screenshots, manual selection, browser history, browser extensions, and device backups remain outside the app's control. Browser timer throttling can delay the exit attempt, so Quick Exit remains the required clear action when finished. `/mobility` has no shared-device mode or timed exit: it offers explicit saving, reviewed copying/downloads and a manual **Clear this session** control. That control removes working forms and cancels pending writes; separately saved records remain until explicitly deleted or cleaned up after expiry. Clipboard, Downloads, screenshots, print-to-PDF files, browser history, and backups sit outside the app’s clear action.
...
- No public route accepts an original citizen-document upload. The optional account API can receive reviewed structured case/profile data after configuration; the optional adviser can receive a separately reviewed minimal context after activation. Source records for `/demo` remain preloaded fictional fixtures; `/review` reads selected documents locally; and the public `/demo/test-lab` custom-image workbench is browser-local and manual. A separate feature-flagged synthetic model adapter exists in code, but both its enabling flags are false in public production and it must never be used with real citizen material.
